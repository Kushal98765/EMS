...
const EditDepartment = () => {
    ...
    useEffect(() => {
        const fetchDepartments = async () => {
            setDepLoading(true);
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/department/${id}`, {
                    headers: {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            const response = await axios.put(`${process.env.REACT_APP_API_URL}/api/department/${id}`, department, {
                headers: {
                    "Authorization": `Bearer ${localStorage.getItem('token')}`
                }
            })
